			...
			con.query("SELECT currentlevel as currentlevel, starcount as starcount FROM Accounts WHERE usernames = '" + username + "';", function (err, rows, fields) {
				...
				con.query("SELECT * FROM Activities WHERE level = '" + currentLevel + "';", function (err, rows, fields) {
					...
					if (rows.length > 0) {
						...
						res.render('completed', { navUsername: username, navStars: starcount });
					}
				});
...
app.post('/Leaderboards', function (req, res) {
	con.query("SELECT usernames , starcount ,currentlevel FROM accounts ORDER BY starcount DESC", function (err, rows, feilds) {
		if (err) {
			throw err;
		}
		else {
			console.log(rows);
			res.render('leaderboards', { sdata: rows });
		}
		// ,{rows:x}}
	})
...
app.get('/compiler', function (req, res) {
	res.sendFile(__dirname +'/fcompile.html')
})
...
app.post('/compiler', function (req, res) {
	...
	var compiler = spawn('g++', ['code.cpp', '-o', 'code']);
	...
	compiler.on('close', function (data) {
		...
		if (data === 0) {
			...
			execute.stdout.on('data', function (output) {
				res.render('fcompile', { outputmessage: output, startingcode: req.body.code });
			});
